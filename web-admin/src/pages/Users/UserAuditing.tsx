    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 flex flex-col flex-1 min-h-0">
                ...
                <div className="flex-1 overflow-y-auto matrix-scroll min-h-0">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-slate-50/80 text-slate-600 font-bold border-b border-slate-200/50 sticky top-0 backdrop-blur-sm z-10 select-none">
                            <tr>
                                <th className="px-6 py-3.5">Mã Đăng Ký</th>
                                <th className="px-6 py-3.5">Tên Cơ Sở</th>
                            ...
                            {isLoading ? (
                                ...
                                registrations.map((reg) => (
                                    <tr key={reg.id} className="hover:bg-slate-50/40 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-black text-brand-emerald">{reg.venueName}</td>
                                        <td className="px-6 py-4 font-bold text-slate-800">{reg.fullName}</td>
                                        <td className="px-6 py-4 text-slate-500 truncate max-w-[200px]" title={`${reg.ward}, ${reg.district}, ${reg.province}`}>
                                        ...
                                        <td className="px-6 py-4 text-right space-x-2">
                                            <Button variant="ghost" size="sm" onClick={() => openDetail(reg.id)}>Chi Tiết</Button>
                                            {reg.status === 'PENDING' && (
                                                <>
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-10 text-center text-slate-400 font-semibold text-sm">
            ...
            <Modal
                ...
                maxWidth="sm"
                footer={
                    <Button onClick={() => setIsApproveSuccessOpen(false)} className="w-full bg-brand-emerald hover:bg-brand-emerald/90 text-white">
                        Đóng
                    </Button>
                }
            >
            ...
            >
                <div className="space-y-4 py-2">
                    <p className="text-sm text-slate-600 font-medium">Vui lòng nhập lý do từ chối để thông báo cho chủ sân biết (bắt buộc):</p>
                    <textarea
