...
import { ArrowLeft, Save, Ticket, Percent, CalendarDays, Hash, AlertTriangle, CheckCircle2, Image as ImageIcon } from 'lucide-react';
...
export const VoucherForm: React.FC<VoucherFormProps> = ({ voucherId, onBack }) => {
  ...
  return (
    <div className="animate-fadeIn py-6 max-w-4xl mx-auto w-full">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 space-y-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
            ...
            <h3 className="text-lg font-black text-slate-800">Ảnh Banner</h3>
          </div>
          <BannerUploader 
            value={formData.bannerImageUrl} 
            onChange={url => setFormData({...formData, bannerImageUrl: url})} 
            disabled={!!(isEdit && hasInteractions)}
          />
        </div>
