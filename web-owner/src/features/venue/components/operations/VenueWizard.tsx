...
import { OperationsSportModal } from './OperationsSportModal';
...
const WizardInner = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleNext = async () => {
    ...
    if (!isPureEditMode) {
      if (step === 1) {
        ...
        return; // Wait for OperationsSportModal confirm
      } else {
  ...
  const handleSportModalConfirm = async () => {
    setIsSportModalOpen(false);
    if (!isPureEditMode && step === 1) {
            ...
            <>
              {step > 1 && (
                <Button
                  ...
                  disabled={loading}
                  className="font-bold text-xs border border-slate-200 text-slate-600 hover:text-slate-800 flex items-center gap-1.5"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
