...
interface OperationsSportModalProps {
  isOpen: boolean;
  onConfirm: () => void;
}

export const OperationsSportModal = ({
  isOpen,
  onConfirm
}: OperationsSportModalProps) => {

  const { sportId, setSportId, loading } = useVenueWizard();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 select-none">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl mx-auto overflow-hidden animate-slideUp">
        <div className="p-8 space-y-8">
          ...
          <div className={`grid grid-cols-2 md:grid-cols-4 gap-6 ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
