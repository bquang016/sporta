...
export const OtpSignatureModal = ({
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      onSuccess({ timestamp, ip: '192.168.1.1' }); // Mock IP for now, real IP is handled by backend
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl p-6 lg:p-8 w-full max-w-sm shadow-2xl animate-[stampIn_0.3s_ease-out]">
        ...
        <div className="flex gap-3">
          ...
          {/* Action button if needed, but it auto-submits, so just a loading state button */}
          {isLoading && (
            <button
              disabled
