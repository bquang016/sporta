...
export const VenueInfoStep = ({
  ...
  const handleCoverChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      onVenueInfoChange({ ...venueInfo, coverImage: file as any });
    }
  };
  ...
  const handleDetailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      onVenueInfoChange({ 
        ...venueInfo, 
        detailImages: [...venueInfo.detailImages, ...Array.from(files)] as any 
      });
    }
  };
          ...
          <div className="flex gap-4 items-start">
            <div className="w-28 h-20 bg-slate-100 border-2 border-dashed border-slate-200 rounded-xl overflow-hidden relative flex-shrink-0 flex items-center justify-center">
              {venueInfo.coverImage ? (
                <img src={URL.createObjectURL(venueInfo.coverImage as any)} alt="Cover preview" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              {venueInfo.detailImages.map((file, index) => (
                <div key={index} className="aspect-video bg-white rounded-xl border border-slate-200 overflow-hidden relative group">
                  <img src={URL.createObjectURL(file as any)} alt={`Detail ${index}`} className="w-full h-full object-cover" />
                  <button
                    type="button"
