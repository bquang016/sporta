  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-100 overflow-hidden animate-fadeIn">
      ...
      <div className="shrink-0 h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-sm z-10 print:hidden">
        ...
        <button
          onClick={onClose}
          className="w-10 h-10 rounded-full flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 matrix-scroll bg-slate-100/50 print:p-0 print:bg-white" ref={printRef}>
          ...
          {isContractSigned && (
            <div className="mt-6 bg-emerald-50 p-6 rounded-xl border border-emerald-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 print:hidden animate-fadeIn">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  onClick={handlePrint}
                  className="flex-1 sm:flex-none px-4 py-2 bg-white border border-emerald-300 text-emerald-700 font-bold text-xs rounded-lg hover:bg-emerald-100 transition-colors"
                >
                  Tải về (PDF)
                </button>
                <button
                  onClick={onClose}
                  className="flex-1 sm:flex-none px-6 py-2 bg-emerald-600 text-white font-bold text-xs rounded-lg hover:bg-emerald-700 transition-colors shadow-sm"
                >
                  Đóng & Tiếp tục
                </button>
              </div>
      ...
      {/* ── FOOTER ── */}
      {!isContractSigned && (
        <div className="shrink-0 p-4 bg-white border-t border-slate-200 flex justify-end gap-3 print:hidden">
          <button
            onClick={onClose}
          ...
          <button
            ...
            className="px-8 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md transition-colors disabled:bg-slate-300 disabled:text-slate-500 disabled:cursor-not-allowed flex items-center gap-2"
          >
