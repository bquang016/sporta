...
interface VenueOperatingStepProps {
  venueInfo: VenueInfo;
...
export const VenueOperatingStep = ({
  venueInfo,
  onVenueInfoChange,
  courts,
  onCourtsChange,
  isLoading
}: VenueOperatingStepProps) => {
  ...
  const [modifierValue, setModifierValue] = useState('20');
  ...
  const handleAddPriceRule = () => {
    ...
    if (ruleType === 'SHIFT') {
      ...
      if (modifierType === 'percentage') {
        percentageModifier = 1.0 + (valNum / 100);
      } else {
        fixedModifier = valNum;
      }
    ...
    const updatedCourts = courts.map((c, idx) => {
      if (idx === selectedCourtIndex) {
        return {
          ...c,
          priceRules: [...(c.priceRules || []), newRule]
        };
      }
      return c;
    });
  ...
  const handleRemovePriceRule = (ruleIndex: number) => {
    const updatedCourts = courts.map((c, idx) => {
      if (idx === selectedCourtIndex) {
        return {
          ...c,
          priceRules: c.priceRules.filter((_, i) => i !== ruleIndex)
        };
      }
      return c;
    });
  ...
  return (
    <div className={`flex-grow overflow-y-auto px-8 py-6 select-none max-w-6xl mx-auto w-full font-sans animate-fadeIn ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-2xs space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Dropdown
                  ...
                  onChange={val => updateField('openingTime', val)}
              ...
              <div className="space-y-1.5">
                ...
                <Dropdown
                  ...
                  onChange={val => updateField('closingTime', val)}
                  className="w-full text-xs font-bold text-slate-700 rounded-xl"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Dropdown
                ...
                value={String(venueInfo.shiftDurationMinutes)}
                onChange={val => updateField('shiftDurationMinutes', parseInt(val))}
                className="w-full text-xs font-bold text-slate-700 rounded-xl"
              />
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-2xs space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
              ...
              <Checkbox
                ...
                onChange={val => updateField('hasSurcharge', val)}
              />
            </div>
              ...
              <div className="space-y-4 animate-slideDown">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={e => updateField('surchargeAmount', e.target.value ? parseFloat(e.target.value) : undefined)}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={e => updateField('surchargeDescription', e.target.value)}
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200/80 rounded-3xl p-5 shadow-2xs space-y-5">
          ...
          {courts.length === 0 ? (
            <div className="border border-dashed border-slate-200 rounded-3xl p-8 text-center text-xs text-slate-400 font-semibold">
              Vui lòng quay lại bước trước để đăng ký ít nhất một sân trực thuộc.
            </div>
          ) : (
