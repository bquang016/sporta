...
const SingleToast = ({ item, onDismiss, index }: SingleToastProps) => {
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden min-w-[280px] max-w-[360px] border rounded-2xl shadow-xl backdrop-blur-md flex items-start gap-3 select-none transition-all duration-300 ease-out font-sans',
        bgMap[item.type],
        translateY
      )}
    ...
    >
      ...
      <div className="flex items-start gap-3 p-4 pr-3 w-full">
        ...
        <button
          ...
          className="flex-shrink-0 p-1 rounded-lg hover:bg-white/15 active:bg-white/25 transition-all text-white/70 hover:text-white mt-0.5 cursor-pointer focus:outline-none"
        >
        ...
        >
          {toasts.map((item, index) => (
            <div key={item.id} className="pointer-events-auto">
              <SingleToast item={item} onDismiss={dismissToast} index={index} />
            </div>
          ))}
        </div>,
        document.body
...
export const useToast = (): ToastContextValue => {
  ...
  if (!ctx) {
    return {
      showToast: (type, message) => console.warn(`[Toast Provider not found] ${type}: ${message}`),
      dismissToast: () => {},
    };
  }
  return ctx;
...
// ─────────────────────────────────────────────────────────────────────────────
// LEGACY STANDALONE TOAST (backward compatible)
// ─────────────────────────────────────────────────────────────────────────────
...
export const Toast = ({ message, type = 'success', onClose, duration = 4000 }: ToastProps) => {
  ...
  return (
    <div
      className={cn(
        'fixed top-4 right-4 z-[9999] px-4 py-3 rounded-2xl shadow-xl border backdrop-blur-md flex items-center gap-3 select-none animate-fadeIn font-sans',
        bgMap[type]
      )}
    ...
    >
      ...
      <span className="text-[11px] font-black tracking-tight leading-normal uppercase text-white">
        {message}
      </span>
      {onClose && (
        <button
          onClick={onClose}
          className="ml-2 p-1 rounded-lg hover:bg-white/10 active:bg-white/20 transition-all text-white/70 hover:text-white cursor-pointer focus:outline-none"
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" />
          </svg>
